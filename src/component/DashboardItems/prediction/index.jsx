...
const handleSubmit=()=>{
  let symptoms1=Object.values(state);
  let sumarr=Object.values(state).reduce((a, b) => a + b, 0);
  if(sumarr===0){
    alert("you must check at least one symptom")
    setError(true);
  }else{
    setError(false);
    ...
    .then(function (response) {
      if(response.status===200){
        //axios illness set in session storage
    ...
    window.sessionStorage.setItem("diseaseId",20);
    window.sessionStorage.setItem("diseaseName","Corona virus");
    alert("You are Probably suffering from corona virus ,You will be redirected to articles conserning this disease and some doctors")
    navigate("/articles")
  }
}
  return (
    <div>
      <Typography>Please check the symptoms you may have , our model will predict your illness</Typography>
      <Box sx={{ display: "flex" }}>
        <FormControl sx={{ m: 3 }} component="fieldset" variant="standard" error={error}>
          <FormLabel component="legend">Disease symptoms</FormLabel>
          <FormGroup >
